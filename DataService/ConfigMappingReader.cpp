...
void ConfigMappingReader::fileLineRead (QString lineReaded) {
    ...
    if (QString::compare("limitsMonVirtual", fields[0], Qt::CaseInsensitive) == 0)
        ...
        std::cerr << "Element unknown by reading configMapping" << fields[0].toStdString() << std::endl;
}
...
void ConfigMappingReader::mappingTypeFound(QStringList fields) {
    ...
    if(!found){
        std::cerr << "Error de sintaxi en el tipus de dades" << std::endl;
    }
}
...
void ConfigMappingReader::baseFound(QStringList fields) {
    ...
    if (QString::compare("fittedPlane", fields[1], Qt::CaseInsensitive) == 0) {
...
void ConfigMappingReader::propFound(QStringList fields) {
    ...
    numProp++;
    //preguntar si hay que meter todos los colores?
    ...
    if (QString::compare("cilindre", fields[4], Qt::CaseInsensitive) == 0) {
        auto tipusObj = ObjectFactory::OBJECT_TYPES::CILINDRE;
        if (QString::compare("COLOR_MAP_TYPE_INFERNO", fields[5], Qt::CaseInsensitive) == 0) {
            auto tipusColorMap = ColorMapStatic::COLOR_MAP_TYPE_INFERNO;
            props.push_back(std::make_pair(tipusObj, tipusColorMap));
        }
    }
    if (QString::compare("sphereLattice", fields[4], Qt::CaseInsensitive) == 0) {
        auto tipusObj = ObjectFactory::OBJECT_TYPES::SPHERE_LATTICE;
        if (QString::compare("COLOR_MAP_TYPE_INFERNO", fields[5], Qt::CaseInsensitive) == 0) {
            auto tipusColorMap = ColorMapStatic::COLOR_MAP_TYPE_INFERNO;
            props.push_back(std::make_pair(tipusObj, tipusColorMap));
        }
    ...
    if (QString::compare("brobject", fields[4], Qt::CaseInsensitive) == 0) {
        auto tipusObj = ObjectFactory::OBJECT_TYPES::BR_OBJECT;
        if (QString::compare("COLOR_MAP_TYPE_INFERNO", fields[5], Qt::CaseInsensitive) == 0) {
            auto tipusColorMap = ColorMapStatic::COLOR_MAP_TYPE_INFERNO;
            props.push_back(std::make_pair(tipusObj, tipusColorMap));
        }
